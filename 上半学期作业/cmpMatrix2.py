...
import os
def main():
    filelist = os.listdir('/home/ppw/Documents/pycode/commit_dot')
    ...
    for iii in range(0, len(filelist)):
        ...
        filename = '/home/ppw/Documents/pycode/commit_dot/' + filelist[iii]
        # open 2 files
        ...
        # write matrix
        matrixpath = '/home/ppw/Documents/pycode/matrixes1/' + filelist[iii] + '.txt'
        string = ''
        with open(matrixpath, 'w') as fff:
            for i in range(0, len(req)):

                for j in range(0, len(req)):
                    string = string + (str(reqmatrix2[i * len(req) + j]) + str(' '))
                string = (string + '\n')
            fff.write(string)
        ...
        print(filename + str(':'))
        print('cosine similirity : ' + str(cosine))

        if cosine != 1:
            k = cosine / math.acos(cosine)
        else:
            k = 0
        # k = math.acos(cosine)/math.pi

        print('k value in y=k*x  : ' + str(k))
        print('\n\n')
